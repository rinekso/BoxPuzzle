...
{
    [CreateAssetMenu(menuName = Paths.ScriptablePath.Map)]
...
#pragma warning restore 0414
        ...
        public static MapSettings Create(string directory, string filename, GridType grid, MapRules rules, Material material)
        {
            var mapPath = string.Format("{0}/{1}", directory, string.Concat(filename, "_", Suffix, grid.ToString(), ".asset"));
            mapPath = AssetDatabase.GenerateUniqueAssetPath(mapPath);
            var mapInstance = ScriptableObject.CreateInstance<MapSettings>();
            mapInstance.Init(grid, rules, material);
